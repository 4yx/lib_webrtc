...
constexpr size_t kMaxBufferedFrames = 48000 * 2;
constexpr int kCaptureFrequency = 48000;
constexpr int kCapturePollMs = 10;
constexpr int kCaptureFramesPerPoll = kCaptureFrequency * kCapturePollMs / 1000;
...
public:
	void pushSamples(
		...
		if (srcChannels >= 2) {
			if (_buffer.size() + frames > kMaxBufferedFrames) {
				const auto overflow = (_buffer.size() + frames)
					- kMaxBufferedFrames;
				_buffer.erase(
					_buffer.begin(),
					_buffer.begin() + overflow);
			}
			for (size_t i = 0; i < frames; ++i) {
				auto mixed = (static_cast<int32_t>(src[i * srcChannels])
	...
	void readAndMix(
		...
		for (size_t i = 0; i < available; ++i) {
			const auto loopbackSample = static_cast<int32_t>(_buffer[i]);
			for (size_t ch = 0; ch < channels; ++ch) {
				const auto idx = i * channels + ch;
				auto mixed = static_cast<int32_t>(destination[idx])
					+ loopbackSample;
				if (mixed > 32767) {
					mixed = 32767;
				} else if (mixed < -32768) {
...
public:
	DirectLoopbackCapture(std::shared_ptr<LoopbackCollector> collector)
...
private:
	static std::string findMonitorDevice() {
		...
		if (defaultPlayback) {
			auto playbackLower = std::string(defaultPlayback);
			constexpr auto kPrefix = "OpenAL Soft on ";
			constexpr auto kPrefixSize = sizeof(kPrefix) - 1;
			if (playbackLower.rfind(kPrefix, 0) == 0) {
				playbackLower = playbackLower.substr(kPrefixSize);
			}
			for (auto &ch : playbackLower) {
				ch = char(std::tolower(static_cast<unsigned char>(ch)));
			}
			for (const auto &id : all) {
				...
				if (lower.find(playbackLower) != std::string::npos) {
					return id;
				}
			}
		}
	...
	void captureLoop(const std::string &deviceId) {
		...
		readBuffer.resize(kCaptureFramesPerPoll * channels);
		...
		while (!_shouldStop.load()) {
			...
			while (samples >= kCaptureFramesPerPoll) {
				alcCaptureSamples(
					device,
					readBuffer.data(),
...
public:
	...
	int32_t RecordedDataIsAvailable(
		...
		if (_microphoneMuted.load(std::memory_order_relaxed)) {
			std::memset(
				_mixBuffer.data(),
				0,
	...
	int32_t NeedMorePlayData(
		...
		const auto result = _inner->NeedMorePlayData(
			...
			elapsed_time_ms,
			ntp_time_ms);
		const auto volume = _playbackVolume.load(std::memory_order_relaxed);
		if (result == 0 && volume < 0.999f && nBytesPerSample == 2) {
			const auto totalSamples = nSamplesOut * nChannels;
...
public:
	...
	void setLoopbackEnabled(bool enabled) {
		...
		if (_mixingTransport) {
			_mixingTransport->setMixingEnabled(enabled);
		}
	}
	...
	int32_t RegisterAudioCallback(
		...
		if (audioCallback) {
			...
			return _inner->RegisterAudioCallback(
				_mixingTransport.get());
		}
...
private:
	...
	public:
		LoopbackAdmTransport(std::shared_ptr<LoopbackCollector> collector)
		: _collector(std::move(collector)) {
		}
